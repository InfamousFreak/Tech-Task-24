...
/*func AdminAuthMiddleware() fiber.Handler {
    ...
    return func(c *fiber.Ctx) error {
        ...
        if authHeader != "" && strings.HasPrefix(authHeader, "Bearer ") {
            ...
            // If the token is not in the Authorization header, check for a cookie named "adminToken"
            tokenString = c.Cookies("adminToken")
            if tokenString == "" {
                return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
                    "error": true,
                    "message": "Missing or malformed JWT",
        ...
        claims, err := parseAndVerifyAdminToken(tokenString, jwtKey)
        if err != nil {
            return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
                "error": true,
                "message": "Invalid or expired JWT: " + err.Error(),
        ...
        // Token is valid and user is an admin, store the claims in the context
        c.Locals("claims", claims)
        return c.Next()
    }
}

func parseAndVerifyAdminToken(tokenString string, jwtKey []byte) (jwt.MapClaims, error) {
    token, err := jwt.Parse(tokenString, func(token *jwt.Token) (interface{}, error) {
        if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
            return nil, fiber.NewError(fiber.StatusUnauthorized, "Invalid token signing method")
        }
